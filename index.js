...
class DeviceHeartbeatBot {
    ...
    constructor(account, proxyConfig = null) {
        ...
        if (proxyConfig) {
            ...
            console.log(chalk.blue(`[馃摗] Running with proxy: ${proxyConfig}`));

        } else {

            console.log(chalk.yellow(`[鈿狅笍] Running without proxy`));
    ...
    logSuccess(action, data) {
        ...
        console.log(chalk.green(`[鉁擼 ${action} Success:`), data);
    ...
    logError(action, error) {
        ...
        console.error(chalk.red(`[鉁朷 ${action} Error:`), error.message || error);
    ...
    logWalletDetails(details) {
        ...
        console.log('\n' + chalk.white(`馃搳 Wallet Details for ${this.account.walletAddress}:`));
    ...
    return new Promise((resolve) => {
        ...
        rl.question('> ', async (answer) => {
            ...
            if (answer.toLowerCase() === 'y') {
                ...
                console.log(chalk.yellow('\n[鈿狅笍] Warning: Using proxies may cause Cloudflare errors'));
    ...
    try {
        ...
        if (useProxy) {
            ...
            if (proxies.length === 0) {
                ...
                console.log(chalk.yellow('[鈿狅笍] No proxies found in proxy.txt, running without proxy'));
